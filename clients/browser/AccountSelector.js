class AccountSelector extends Krypton.Observable {
    constructor(el, $) {
        super();
    ...
    get selectedAddress() {
        try {
            return Krypton.Address.fromUserFriendlyAddress(this.$el.value);
        } catch(e) {
            return null;
        }
    }
    ...
    _getAddressesFromLocalStorage(key) {
        return new LocalStorageList(key).get().map(userFriendly => Krypton.Address.fromUserFriendlyAddress(userFriendly));
    }
