class MultiSigSignerUi {
    ...
    _preselectSigners(multiSigWallet) {
        ...
        this.$.walletStore.list().then(walletAddresses => {
            ...
            const combinations = [...Krypton.ArrayUtils.k_combinations(publicKeys, multiSigWallet.minSignatures)];
            const validCombination = combinations.find(combination => {
                const aggregatedPublicKey = Krypton.PublicKey.sum(combination);
