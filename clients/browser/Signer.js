...
class SingleSigSigner extends Signer {
    ...
    get type() {
        return Krypton.Account.Type.BASIC;
    }
...
class MultiSigSigner extends Signer {
    ...
    constructor($, address, signingAddresses) {
        ...
        return super($, address)
            .then(instance => {
                ...
                _this._aggregatedSigningPublicKey = Krypton.PublicKey.sum(_this._signingPublicKeys);
                _this._signingWallets = signingKeyPairs.map(keyPair =>
                    new Krypton.MultiSigWallet(keyPair, signingAddresses.length, _this._combinationsPublicKeys));
                return _this;
            });
    }
    ...
    sign(tx) {
        ...
        const aggregatedCommitment = Krypton.Commitment.sum(commitmentPairs.map(pair => pair.commitment));
        ...
        const signature = Krypton.Signature.fromPartialSignatures(aggregatedCommitment, partialSignatures);
        const proof = Krypton.SignatureProof.multiSig(this._aggregatedSigningPublicKey, this._combinationsPublicKeys,
            signature).serialize();
        return {
            signature: signature,
            proof: proof
...
class VestingSigner extends Signer {
    ...
    get type() {
        return Krypton.Account.Type.VESTING;
    }
...
class HtlcSigner extends Signer {
    /** @async */
    constructor($, address, proofType = Krypton.HashedTimeLockedContract.ProofType.REGULAR_TRANSFER,
                proofHashAlgorithm = Krypton.Hash.Algorithm.BLAKE2B, proofHashPreImage = '',
    ...
    get type() {
        return Krypton.Account.Type.HTLC;
    }
    ...
    sign(tx) {
        ...
        const proof = new Krypton.SerialBuffer(proofSize);

        proof.writeUint8(this._proofType);

        if (this._proofType === Krypton.HashedTimeLockedContract.ProofType.REGULAR_TRANSFER) {
            let hash = Krypton.BufferUtils.fromAscii(this._proofHashPreImage); // ascii preimage
    ...
    _getSigners(htlcSenderSigner = null, htlcRecipientSigner = null) {
        ...
        if (this._proofType === Krypton.HashedTimeLockedContract.ProofType.REGULAR_TRANSFER
            || this._proofType === Krypton.HashedTimeLockedContract.ProofType.EARLY_RESOLVE) {
        ...
        if (this._proofType === Krypton.HashedTimeLockedContract.ProofType.EARLY_RESOLVE
            || this._proofType === Krypton.HashedTimeLockedContract.ProofType.TIMEOUT_RESOLVE) {
    ...
    _calculateTotalProofSize(proofType, hashAlgorithm, signatureProofs) {
        const expectedProofCount = proofType === Krypton.HashedTimeLockedContract.ProofType.EARLY_RESOLVE? 2 : 1;
        ...
        if (proofType === Krypton.HashedTimeLockedContract.ProofType.REGULAR_TRANSFER) {
            return /* proof type */ 1
                + /* algorithm */ 1
                + /* hash depth */ 1
                + /* hash root */ Krypton.Hash.getSize(hashAlgorithm)
                + /* hashed pre image */ Krypton.Hash.getSize(hashAlgorithm)
