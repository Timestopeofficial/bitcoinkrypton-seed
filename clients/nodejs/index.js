...
if ((config.protocol === 'wss' && !(config.host && config.port && config.tls && config.tls.cert && config.tls.key)) ||
    ...
    console.log(
        ...
        '           [:PASSWORD]       PORT (default: 12411). If PASSWORD is specified, it\n' +
        '                             is required to be used for username "metrics" via\n' +
        '                             Basic Authentication.\n' +
        '  --ui[=PORT]                Serve a UI on port PORT (default: 12311).\n' +
        ...
        '                 [,IP]       (default: 12111,::ffff:127.0.0.1).\n' +
...
if (!Krypton.GenesisConfig.CONFIGS[config.network]) {
...
Krypton.Log.instance.level = config.log.level;
for (const tag in config.log.tags) {
    Krypton.Log.instance.setLoggable(tag, config.log.tags[tag]);
}

for (const key in config.constantOverrides) {
    Krypton.ConstantHelper.instance.set(key, config.constantOverrides[key]);
}
...
(async () => {
    if (config.protocol === 'dumb') {
        Krypton.Log.e(TAG, `******************************************************************************`);
        Krypton.Log.e(TAG, `*                                                                            *`);
        Krypton.Log.e(TAG, `*  You are running in 'dumb' configuration, so others can't connect to you.  *`);
        Krypton.Log.e(TAG, `*  Consider switching to a proper WebSocket/WebSocketSecure configuration.   *`);
        Krypton.Log.e(TAG, `*                                                                            *`);
        Krypton.Log.e(TAG, `******************************************************************************`);
    }

    Krypton.Log.i(TAG, `Krypton NodeJS Client starting (network=${config.network}`
    ...
    Krypton.GenesisConfig.init(Krypton.GenesisConfig.CONFIGS[config.network]);

    for (const seedPeer of config.seedPeers) {
        let address;
        switch (seedPeer.protocol) {
            case 'ws':
                address = Krypton.WsPeerAddress.seed(seedPeer.host, seedPeer.port, seedPeer.publicKey);
                break;
            case 'wss':
            default:
                address = Krypton.WssPeerAddress.seed(seedPeer.host, seedPeer.port, seedPeer.publicKey);
                break;
        }
        Krypton.GenesisConfig.SEED_PEERS.push(address);
    }

    const clientConfigBuilder = Krypton.Client.Configuration.builder();
    ...
    if (config.passive) clientConfigBuilder.feature(Krypton.Client.Feature.PASSIVE);
    if (config.type === 'full' || config.type === 'light') clientConfigBuilder.feature(Krypton.Client.Feature.MEMPOOL);
    const clientConfig = clientConfigBuilder.build();
    const networkConfig = clientConfig.networkConfig;

    switch (config.type) {
        case 'full':
            $.consensus = await Krypton.Consensus.full(networkConfig);
            break;
        case 'light':
            $.consensus = await (!config.volatile
                ? Krypton.Consensus.light(networkConfig)
                : Krypton.Consensus.volatileLight(networkConfig));
            break;
        case 'nano':
            $.consensus = await (!config.volatile
                ? Krypton.Consensus.nano(networkConfig)
                : Krypton.Consensus.volatileNano(networkConfig));
            break;
        case 'pico':
            $.consensus = await Krypton.Consensus.pico(networkConfig);
    }

    $.client = new Krypton.Client(clientConfig, $.consensus);
    $.blockchain = $.consensus.blockchain;
    $.accounts = $.blockchain.accounts;
    $.mempool = $.consensus.mempool;
    $.network = $.consensus.network;

    Krypton.Log.i(TAG, `Peer address: ${networkConfig.peerAddress.toString()} - public key: ${networkConfig.keyPair.publicKey.toHex()}`);

    // TODO: Wallet key.
    $.walletStore = await new Krypton.WalletStore();
    if (!config.wallet.address && !config.wallet.seed) {
        ...
        const mainWallet = Krypton.Wallet.loadPlain(config.wallet.seed);
        await $.walletStore.put(mainWallet);
        await $.walletStore.setDefault(mainWallet.address);
        $.wallet = mainWallet;
    } else {
        const address = Krypton.Address.fromUserFriendlyAddress(config.wallet.address);
    ...
    Krypton.Log.i(TAG, `Managing wallets [${addresses.map(address => address.toUserFriendlyAddress())}]`);

    const account = await $.client.getAccount($.wallet.address).catch(() => null);
    Krypton.Log.i(TAG, `Wallet initialized for address ${$.wallet.address.toUserFriendlyAddress()}.`
        + (account ? ` Balance: ${Krypton.Policy.satoshisToCoins(account.balance).toString()} BTCK` : ''));

    const chainHeight = await $.client.getHeadHeight();
    const chainHeadHash = await $.client.getHeadHash();
    Krypton.Log.i(TAG, `Blockchain state: height=${chainHeight}, headHash=${chainHeadHash}`);

    const extraData = config.miner.extraData ? Krypton.BufferUtils.fromAscii(config.miner.extraData) : new Uint8Array(0);
    if (config.poolMining.enabled || config.uiServer.enabled) { // ui requires SmartPoolMiner to be able to switch
        ...
        const deviceId = Krypton.BasePoolMiner.generateDeviceId(networkConfig);
        const deviceData = config.poolMining.deviceData || {};
        const poolMode = isNano ? 'nano' : config.poolMining.mode;
        deviceData.userAgent = Krypton.Version.createUserAgent();
        switch (poolMode) {
            case 'nano':
                $.miner = new Krypton.NanoPoolMiner($.blockchain, $.network.time, $.wallet.address, deviceId, deviceData);
                break;
            case 'smart':
            default:
                $.miner = new Krypton.SmartPoolMiner($.blockchain, $.accounts, $.mempool, $.network.time, $.wallet.address, deviceId, deviceData, extraData);
                break;
        }
        $.client.addConsensusChangedListener((state) => {
            if (state === Krypton.Client.ConsensusState.ESTABLISHED) {
                ...
                if (!config.poolMining.host || config.poolMining.port === -1) {
                    Krypton.Log.i(TAG, 'Not connecting to pool as mining pool host or port were not specified.');
                    return;
                }
                Krypton.Log.i(TAG, `Connecting to pool ${config.poolMining.host} using device id ${deviceId} as a ${poolMode} client.`);
                $.miner.connect(config.poolMining.host, config.poolMining.port);
            }
        });
    } else {
        $.miner = new Krypton.Miner($.blockchain, $.accounts, $.mempool, $.network.time, $.wallet.address, extraData);
    }

    let consensusState = Krypton.Client.ConsensusState.CONNECTING;
    $.client.addConsensusChangedListener(async (state) => {
        consensusState = state;
        if (state === Krypton.Client.ConsensusState.ESTABLISHED) {
            if (config.miner.enabled) $.miner.startWork();
            Krypton.Log.i(TAG, `Blockchain ${config.type}-consensus established in ${(Date.now() - START) / 1000}s.`);
            const chainHeight = await $.client.getHeadHeight();
            const chainHeadHash = await $.client.getHeadHash();
            Krypton.Log.i(TAG, `Current state: height=${chainHeight}, headHash=${chainHeadHash}`);
        } else {
    ...
    $.client.addBlockListener(async (hash) => {
        if (consensusState === Krypton.Client.ConsensusState.SYNCING) {
            const head = await $.client.getBlock(hash, false);
            if (head.height % 100 === 0) {
                Krypton.Log.i(TAG, `Syncing at block: ${head.height}`);
            }
        }
    });
    ...
    $.client.addHeadChangedListener(async (hash, reason) => {
        const head = await $.client.getBlock(hash, false);
        Krypton.Log.i(TAG, `Now at block: ${head.height} (${reason})`);
    });

    // TODO: Peer changed listeners
    $.network.on('peer-joined', (peer) => {
        Krypton.Log.i(TAG, `Connected to ${peer.peerAddress.toString()}`);
    });
    $.network.on('peer-left', (peer) => {
        Krypton.Log.i(TAG, `Disconnected from ${peer.peerAddress.toString()}`);
    });

    const isSeed = (peerAddress) => Krypton.GenesisConfig.SEED_PEERS.some(seed => seed.equals(peerAddress));
    $.network.on('peer-joined', (peer) => {
        if (Math.abs(peer.timeOffset) > Krypton.Network.TIME_OFFSET_MAX && isSeed(peer.peerAddress)) {
            Krypton.Log.e(TAG, 'Your local system time seems to be wrong! You might not be able to synchronize with the network.');
        }
    });
    ...
    $.miner.on('block-mined', (block) => {
        Krypton.Log.i(TAG, `Block mined: #${block.header.height}, hash=${block.header.hash()}`);
    });
    ...
    if (config.statistics > 0) {
        ...
        $.miner.on('hashrate-changed', async (hashrate) => {
            ...
            if (hashrates.length >= outputInterval) {
                ...
                Krypton.Log.i(TAG, `Hashrate: ${(sum / hashrates.length).toFixed(2).padStart(7)} H/s`
                    + (account ? ` - Balance: ${Krypton.Policy.satoshisToCoins(account.balance).toString()} BTCK` : '')
                    + (config.poolMining.enabled ? ` - Pool balance: ${Krypton.Policy.satoshisToCoins($.miner.balance).toString()} BTCK (confirmed ${Krypton.Policy.satoshisToCoins($.miner.confirmedBalance).toString()} BTCK)` : '')
    ...
    if (config.uiServer.enabled) {
        ...
        openBrowserTab(`http://localhost:${config.uiServer.port}#port=${config.rpcServer.port}`, () => {
            Krypton.Log.w(TAG, 'Failed to automatically open the UI in your web browser.');
            Krypton.Log.w(TAG, `Go to http://localhost:${config.uiServer.port}#port=${config.rpcServer.port} to access it.`);
        });
    }
})().catch(e => {
    console.error(e);
