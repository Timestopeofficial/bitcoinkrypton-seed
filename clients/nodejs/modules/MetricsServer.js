...
class MetricsServer {
    ...
    _measureMessage(msg, time, size) {
        if (!Krypton.PeerChannel.Event[msg.type]) return;
        const str = Krypton.PeerChannel.Event[msg.type];
    ...
    async _mempoolMetrics(res) {
        ...
        if (!this._client._config.hasFeature(Krypton.Client.Feature.MEMPOOL)) return;
    ...
    async _networkMetrics(res) {
        ...
        for (const peer of (await this._client.network.getPeers())) {
            ...
            switch (peer.peerAddress ? peer.peerAddress.protocol : -1) {
                case Krypton.Protocol.DUMB:
                    o.type = 'dumb';
                    break;
                case Krypton.Protocol.WSS:
                    o.type = 'websocket-secure';
                    break;
                case Krypton.Protocol.RTC:
                    o.type = 'webrtc';
                    break;
                case Krypton.Protocol.WS:
                    o.type = 'websocket';
                    break;
                default:
                    o.type = 'unknown';
            ...
            switch (peer.state) {
                ...
                case Krypton.PeerConnectionState.ESTABLISHED:
                    o.state = 'established';
                    break;
                case Krypton.PeerConnectionState.CLOSED:
                    o.state = 'closed';
            }
