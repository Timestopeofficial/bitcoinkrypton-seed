class FactsUI {
    ...
    set averageBlockReward(satoshis) {
        if (!satoshis) {
            this._averageBlockReward.textContent = '0 BTCK';
            return;
        }
        const nims = Krypton.Policy.satoshisToCoins(satoshis);
        if (nims.lt(1e-5)) {
            this._averageBlockReward.textContent = satoshis.toFixed(6) + ' Satoshi';
        } else {
            this._averageBlockReward.textContent = nims.toFixed(6) + ' BTCK';
        }
    }
    ...
    set myBalance(balance) {
        this._myBalance.textContent = Krypton.Policy.satoshisToCoins(balance).toFixed(6);
    }

    set poolBalance(balance) {
        this._poolBalance.textContent = Krypton.Policy.satoshisToCoins(balance).toFixed(6);
    }

    set address(address) {
        const blockExplorerUrl = 'https://explorer.bitcoinkrypton.org/';
    ...
    set blockReward(satoshis) {
        this._blockReward.textContent = Krypton.Policy.satoshisToCoins(satoshis).toFixed(3);
    }
...
class Miner {
    ...
    get globalHashrate() {
        return this.$.blockchain.head.difficulty * Math.pow(2, 16) / Krypton.Policy.blockTime(this.$.blockchain.height);
    }
    ...
    _onConsensusEstablished() {
        ...
        this.ui.facts.blockReward = Krypton.Policy.blockRewardAt(this.$.blockchain.height);
        this.ui.reconnected();
    ...
    _onPoolMinerConnectionChange() {
        ...
        if (state === Krypton.BasePoolMiner.ConnectionState.CONNECTED) {
            this.ui.facts.poolBalance = this.$.miner.confirmedBalance || new Krypton.BigNumber(0);
            this.ui.poolMinerCanConnect();
        } else if (state === Krypton.BasePoolMiner.ConnectionState.CLOSED
            && this.poolEnabled
            && this._previousPoolConnectionState === Krypton.BasePoolMiner.ConnectionState.CONNECTING) {
            // connecting failed
    ...
    _onAverageBlockRewardChanged() {
        ...
        this.ui.facts.averageBlockReward =
            Math.min(1, this.hashrate / this.globalHashrate) * Krypton.Policy.blockRewardAt(this.$.blockchain.height);
    }

    _onExpectedHashTimeChanged() {
        if (this.poolEnabled) return;
        const myWinProbability = this.hashrate / this.globalHashrate;
        this.ui.facts.expectedHashTime = (1 / myWinProbability) * Krypton.Policy.blockTime(this.$.blockchain.height);
    }
    ...
    async _updateBalance() {
        const account = await this.$.accounts.get(this.$.miner.address) || Krypton.BasicAccount.INITIAL;
