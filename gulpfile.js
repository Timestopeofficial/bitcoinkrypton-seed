...
const sources = {
    ...
    generic: [
        ...
        './src/main/generic/consensus/base/account/Bech32.js',
...
gulp.task('build-loader', function () {
    return merge(
        browserify([], {
            ...
            .pipe(buffer())
            .pipe(sourcemaps.init()),
        gulp.src(['./src/main/platform/browser/utils/WindowDetector.js', './src/main/platform/browser/Krypton.js'])
            .pipe(sourcemaps.init())
            .pipe(babel(babel_loader)))
        .pipe(concat('krypton.js'))
        .pipe(uglify(uglify_config))
        .pipe(sourcemaps.write('.'))
        .pipe(gulp.dest('dist'));
});
