...
/* Keccak constants for 24 rounds */
...
void keccak_init(KECCAK_CTX *ctx, unsigned bits) {
	/* NB: The Keccak capacity parameter = bits * 2 */
	unsigned rate = 1600 - bits * 2;

	memzero(ctx, sizeof(KECCAK_CTX));
...
static void keccak_permutation(uint64_t *state) {
	int round = 0;
 ...
 */
static void keccak_process_block(uint64_t hash[25], const uint64_t *block, size_t block_size) {
	/* expanded loop */
 ...
 */
void keccak_update(KECCAK_CTX *ctx, const unsigned char *msg, size_t size) {
	...
	while (size >= block_size) {
		...
		keccak_process_block(ctx->hash, aligned_message_block, block_size);
		msg  += block_size;
		size -= block_size;
	}
	if (size) {
...
*/
void keccak_final(KECCAK_CTX *ctx, unsigned char* result) {
