...
int ripemd160(const unsigned char *data, size_t length, uint8_t *digest)
{
   ...
   int ret;
   if ((ret = ripemd160_init(&context))) return ret;
   if ((ret = ripemd160_update(&context, data, length))) return ret;
   if ((ret = ripemd160_final(&context, digest))) return ret;
   return 0;
}
...
int ripemd160_update(Ripemd160Context *context, const unsigned char *data, size_t length)
{
   ...
   if (context->size > sizeof(context->buffer)) return 1;
