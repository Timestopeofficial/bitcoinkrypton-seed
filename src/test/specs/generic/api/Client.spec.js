describe('Client', () => {
    ...
    established('can handle pending known transactions in getTransactionsByAddress', async (done, client) => {
        ...
        const networkId = 4;
        const newTx = new BasicTransaction(senderPubKey, recipientAddr, value, validityStartHeight, signature, networkId);
    ...
    established('can subscribe to mempool tx updates', async (done, /** @type {Client} */ client) => {
        ...
        const newTx = TestBlockchain.createTransaction(testChain.users[0].publicKey, new Address(a), 1, 1, testChain.users[0].privateKey);
        let handle;
