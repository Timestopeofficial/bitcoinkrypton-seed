describe('HashedTimeLockedContract', () => {
    ...
    it('will deny incoming transaction after creation', () => {
        ...
        let transaction = new ExtendedTransaction(Address.NULL, Account.Type.BASIC, Address.NULL, Account.Type.BASIC, 1000, 1, Transaction.Flag.NONE, new Uint8Array(0));
        expect(() => account.withIncomingTransaction(transaction, 1)).toThrowError();
        transaction = new ExtendedTransaction(Address.NULL, Account.Type.BASIC, Address.NULL, Account.Type.BASIC, 1000, 1, Transaction.Flag.CONTRACT_CREATION, new Uint8Array(0));
        expect(() => account.withIncomingTransaction(transaction, 1)).toThrowError();
    });
    ...
    it('can falsify invalid incoming transaction', (done) => {
        (async () => {
            // No data
            let transaction = new ExtendedTransaction(sender, Account.Type.BASIC, Address.NULL, Account.Type.HTLC, 100, 0, Transaction.Flag.NONE, new Uint8Array(0));
            expect(await HashedTimeLockedContract.verifyIncomingTransaction(transaction)).toBeFalsy();

            // Data too short
            transaction = new ExtendedTransaction(sender, Account.Type.BASIC, Address.NULL, Account.Type.HTLC, 100, 0, Transaction.Flag.NONE, new Uint8Array(18));
            expect(await HashedTimeLockedContract.verifyIncomingTransaction(transaction)).toBeFalsy();
            ...
            transaction = new ExtendedTransaction(sender, Account.Type.BASIC, Address.CONTRACT_CREATION, Account.Type.HTLC, 100, 0, Transaction.Flag.NONE, data);
            expect(await HashedTimeLockedContract.verifyIncomingTransaction(transaction)).toBeFalsy();
            ...
            data.writeUint8(2);
            data.writeUint32(1000);
            transaction = new ExtendedTransaction(sender, Account.Type.BASIC, Address.NULL, Account.Type.HTLC, 100, 0, Transaction.Flag.NONE, data);
            expect(await HashedTimeLockedContract.verifyIncomingTransaction(transaction)).toBeFalsy();
    ...
    it('can falsify invalid outgoing transaction', (done) => {
        (async () => {
            ...
            // No proof
            let transaction = new ExtendedTransaction(sender, Account.Type.HTLC, addr, Account.Type.BASIC, 100, 1, Transaction.Flag.NONE, new Uint8Array(0));
            ...
            transaction = new ExtendedTransaction(sender, Account.Type.HTLC, recipient, Account.Type.BASIC, 100, 0, Transaction.Flag.NONE, new Uint8Array(0));
    ...
    it('can verify valid outgoing transaction', (done) => {
        (async () => {
            const keyPair = KeyPair.generate();
            const addr = keyPair.publicKey.toAddress();
            const transaction = new ExtendedTransaction(sender, Account.Type.HTLC, addr, Account.Type.BASIC, 100, 1, Transaction.Flag.NONE, new Uint8Array(0));
    ...
    it('can redeem funds regularly before timeout', (done) => {
        (async () => {
            ...
            const transaction = new ExtendedTransaction(recipient, Account.Type.HTLC, addr, Account.Type.BASIC, 100, 1, Transaction.Flag.NONE, new Uint8Array(0));
            ...
            expect(account2.balance.eq(900)).toBe(true);
        })().then(done, done.fail);
    });
    ...
    it('blocks regular redemption of too many funds', (done) => {
        (async () => {
            ...
            const addr = keyPair.publicKey.toAddress();
            const hashRoot = Hash.blake2b(Hash.NULL.array);
            const transaction = new ExtendedTransaction(recipient, Account.Type.HTLC, addr, Account.Type.BASIC, 600, 100, Transaction.Flag.NONE, new Uint8Array(0));
            const account = new HashedTimeLockedContract(1001000, sender, addr, hashRoot, 2, 1000, 1000);
            const signatureProof = new SignatureProof(keyPair.publicKey, new MerklePath([]), Signature.create(keyPair.privateKey, keyPair.publicKey, transaction.serializeContent()));
            const proof = new SerialBuffer(3 + 2 * Hash.SIZE.get(Hash.Algorithm.BLAKE2B) + signatureProof.serializedSize);
            proof.writeUint8(HashedTimeLockedContract.ProofType.REGULAR_TRANSFER);
            proof.writeUint8(Hash.Algorithm.BLAKE2B);
    ...
    it('correctly allows to resolve after timeout', (done) => {
        (async () => {
            ...
            const addr = keyPair.publicKey.toAddress();
            const hashRoot = Hash.blake2b(Hash.NULL.array);
            let transaction = new ExtendedTransaction(sender, Account.Type.HTLC, addr, Account.Type.BASIC, 100, 500, Transaction.Flag.NONE, new Uint8Array(0));
            const account = new HashedTimeLockedContract(1001000, addr, recipient, hashRoot, 1, 1000, 1000);
            ...
            transaction = new ExtendedTransaction(sender, Account.Type.HTLC, addr, Account.Type.BASIC, 100, 2000, Transaction.Flag.NONE, new Uint8Array(0));
    ...
    it('can create contract from transaction', (done) => {
        (async () => {
            ...
            data.writeUint8(2);
            data.writeUint32(1000);
            const transaction = new ExtendedTransaction(sender, Account.Type.BASIC, Address.CONTRACT_CREATION, Account.Type.HTLC, 100, 0, Transaction.Flag.CONTRACT_CREATION, data);
            expect(await HashedTimeLockedContract.verifyIncomingTransaction(transaction)).toBeTruthy();
            ...
            expect(contract.balance.eq(100)).toBe(true);
    ...
    it('can plain creation transaction', () => {
        ...
        data.writeUint8(2);
        data.writeUint32(1000);
        const creationTransaction = new ExtendedTransaction(sender, Account.Type.BASIC, Address.CONTRACT_CREATION, Account.Type.HTLC, 100, 0, Transaction.Flag.CONTRACT_CREATION, data);
    ...
    it('can convert itself to and from plain', () => {
        ...
        expect(revived.balance.eq(1e5)).toBe(true);
        ...
        expect(revived.totalAmount.eq(1e5)).toBe(true);
