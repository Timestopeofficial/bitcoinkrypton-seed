describe('BasicAccount', () => {
    ...
    it('can accept incoming transactions', (done) => {
        (async () => {
            ...
            transaction = new ExtendedTransaction(recipient, Account.Type.BASIC, recipient, Account.Type.BASIC, 100, 0, Transaction.Flag.NONE, new Uint8Array(60));
            expect(await BasicAccount.verifyIncomingTransaction(transaction)).toBeTruthy();
        })().then(done, done.fail);
    });
    ...
    it('can deny incoming transactions', (done) => {
        (async () => {
            let transaction = new ExtendedTransaction(recipient, Account.Type.BASIC, recipient, Account.Type.BASIC, 100, 0, Transaction.Flag.NONE, new Uint8Array(65));
            expect(await BasicAccount.verifyIncomingTransaction(transaction)).toBeFalsy();

            transaction = new ExtendedTransaction(recipient, Account.Type.BASIC, recipient, Account.Type.BASIC, 100, 0, Transaction.Flag.NONE, new Uint8Array(1000));
            expect(await BasicAccount.verifyIncomingTransaction(transaction)).toBeFalsy();
        })().then(done, done.fail);
    });
    ...
    it('can falsify invalid outgoing transaction', (done) => {
        (async () => {
            ...
            const transaction = new BasicTransaction(pubKey, recipient, 100, 0, signature);
    ...
    it('can apply outgoing transaction', () => {
        ...
        let transaction = new BasicTransaction(pubKey, recipient, 1, 0);
        account = account.withOutgoingTransaction(transaction, 1, cache);

        expect(account.balance).toBe(99);

        transaction = new BasicTransaction(pubKey, recipient, 50, 1);
    ...
    it('refuses to apply invalid outgoing transaction', () => {
        ...
        const cache = new TransactionCache();
        let transaction = new BasicTransaction(pubKey, recipient, 1, 4);
        expect(() => account.withOutgoingTransaction(transaction, 1, cache)).toThrowError('Validity Error!');

        transaction = new BasicTransaction(pubKey, recipient, 101, 0);
    ...
    it('can revert outgoing transaction', () => {
        ...
        const cache = new TransactionCache();
        const transaction = new BasicTransaction(pubKey, recipient, 1, 0);
        account = account.withOutgoingTransaction(transaction, 1, cache);
