describe('BasicTransaction', () => {
    ...
    it('is correctly created', () => {
        const tx1 = new BasicTransaction(senderPubKey, recipientAddr, value, validityStartHeight, signature, networkId);
    ...
    it('can falsify an invalid signature', () => {
        const tx1 = new BasicTransaction(senderPubKey, recipientAddr, value, validityStartHeight, signature);
    ...
    it('can falsify a transaction from a different network ID', () => {
        ...
        const tx2 = TestBlockchain.createTransaction(users[0].publicKey, users[1].address, 1000, 0, users[0].privateKey, undefined, 0);
        expect(tx2.verify()).toBe(false);
    });

    it('can verify a valid signature', () => {
        const users = TestBlockchain.getUsers(2);
        const tx = TestBlockchain.createTransaction(users[0].publicKey, users[1].address, 1000, 0, users[0].privateKey);
