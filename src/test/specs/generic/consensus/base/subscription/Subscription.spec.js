describe('Subscription', () => {
    ...
    beforeAll((done) => {
        (async () => {
            ...
            tx1 = new BasicTransaction(senderPubKey, recipientAddr, value, nonce, signature);
            tx2 = new ExtendedTransaction(senderAddress, Account.Type.BASIC, recipientAddr, Account.Type.BASIC, value, nonce, Transaction.Flag.NONE, data, proof);

            txHighFee = new BasicTransaction(senderPubKey, recipientAddr, value, nonce, signature);
        })().then(done, done.fail);
    });
    ...
    it('does only allow valid types/addresses', (done) => {
        (async function () {
            ...
            const tx = new BasicTransaction(senderPubKey, recipientAddr, value, nonce, signature);
