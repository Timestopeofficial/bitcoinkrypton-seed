describe('Mempool', () => {
    it('will not push the same transaction twice', (done) => {
        (async function () {
            ...
            const transaction = wallet.createTransaction(Address.unserialize(BufferUtils.fromBase64(Dummy.address1)), 543, 1);
    ...
    it('will always verify a transaction before accepting it', (done) => {
        (async function () {
            ...
            // Create a transaction
            let transaction = wallet.createTransaction(Address.unserialize(BufferUtils.fromBase64(Dummy.address1)), 3523, 1);
            ...
            transaction = wallet.createTransaction(Address.unserialize(BufferUtils.fromBase64(Dummy.address1)), 3523, 3);
            result = await mempool.pushTransaction(transaction);
    ...
    it('can push and get a valid transaction', (done) => {
        (async function () {
            ...
            // Create a transaction
            const referenceTransaction = wallet.createTransaction(Address.unserialize(BufferUtils.fromBase64(Dummy.address1)), 523, 1);
    ...
    it('can push 2 transactions from same user', (done) => {
        (async () => {
            ...
            // Create transactions
            const t1 = wallet.createTransaction(Address.unserialize(BufferUtils.fromBase64(Dummy.address1)), 50, 1);
            const t2 = wallet.createTransaction(Address.unserialize(BufferUtils.fromBase64(Dummy.address1)), 100, 1);
    ...
    it('can get a list of its transactions and can evict them', (done) => {
        (async function () {
            ...
            for (let i = 0; i < numberOfTransactions; i++) {
                const transaction = wallets[i].createTransaction(Address.unserialize(BufferUtils.fromBase64(Dummy.address1)), 234, 1);
    ...
    it('can evict mined transactions', (done) => {
        (async function () {
            ...
            const referenceTransactions = [];
            for (let i = 1; i < 6; i++) {
                const transaction = wallets[0].createTransaction(wallets[i].address, 1, 1);
                const result = await mempool.pushTransaction(transaction); // eslint-disable-line no-await-in-loop
                expect(result).toBe(Mempool.ReturnCode.ACCEPTED);
                referenceTransactions.push(transaction);
            }
    ...
    it('can evict non-mined transactions to restore validity', (done) => {
        (async function () {
            ...
            const largeTransaction = wallets[0].createTransaction(wallets[2].address, 4, 1);
    ...
    it('prefers high fee transactions over low fee transactions', (done) => {
        (async function () {
            ...
            const lowFeeTransaction = wallets[0].createTransaction(wallets[2].address, 1, 1);
            let result = await mempool.pushTransaction(lowFeeTransaction);
            expect(result).toBe(Mempool.ReturnCode.INVALID);

            // Push a higher fee transaction
            const highFeeTransaction = wallets[0].createTransaction(wallets[2].address, 1, 1);
    ...
    it('rejects free transactions beyond the free transaction limit', (done) => {
        (async function () {
            ...
            const lowFeeTransaction = wallets[0].createTransaction(wallets[2].address, 2, 1);
    ...
    it('has a maximum size', (done) => {
        (async function () {
            ...
            for (let i = 0; i < Mempool.SIZE_MAX + 1; i++) {
                const transaction = wallets[i].createTransaction(wallets[(i + 1) % (Mempool.SIZE_MAX + 1)].address, 1, 1);
                const result = await mempool.pushTransaction(transaction); // eslint-disable-line no-await-in-loop
                expect(result).toBe(Mempool.ReturnCode.ACCEPTED);
            }
    ...
    it('can evict by minFeePerByte', (done) => {
        (async function () {
            ...
            const feesPerByte = [];
            for (let i = 0; i < 20; i++) {
                const transaction = wallets[i].createTransaction(wallets[(i + 1) % (20)].address, 1, 1);
