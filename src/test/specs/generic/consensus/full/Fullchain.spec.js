describe('Blockchain', () => {
    it('verifies block size limit', (done) => {
        (async function () {
            ...
            for (let i = 0; i < numTransactions; i++) {
                ...
                transactions.push(TestBlockchain.createTransaction(sender.publicKey, recipient, 1, 1, sender.privateKey));
            }
    ...
    it('verifies transaction signatures', (done) => {
        (async function () {
            ...
            const transactions = [TestBlockchain.createTransaction(senderPubKey, receiverAddr, 1, 0, undefined, wrongSignature)];
    ...
    it('verifies transaction signatures in large blocks', (done) => {
        (async function () {
            ...
            const data = new Uint8Array(32);
            const wrongSignature = Signature.create(testBlockchain.users[0].privateKey, testBlockchain.users[0].publicKey, data);
            transactions.push(TestBlockchain.createTransaction(sender.publicKey, receiver.address, 1, 0, undefined, wrongSignature));
            transactions.sort((a, b) => a.compareBlockOrder(b));
    ...
    it('verifies that sufficient funds are available', (done) => {
        (async function () {
            ...
            const transactions = [TestBlockchain.createTransaction(senderPubKey, receiverAddr, Policy.coinsToSatoshis(1000000), 0, senderPrivKey)];
            const block = await testBlockchain.createBlock({transactions: transactions});
            const status = await testBlockchain.pushBlock(block);
            expect(status).toBe(FullChain.ERR_INVALID);
        })().then(done, done.fail);
    ...
    it('verifies transaction validityStartHeight', (done) => {
        (async function () {
            ...
            const transactions = [TestBlockchain.createTransaction(senderPubKey, receiverAddr, 1, 42, senderPrivKey)];
            const block = await testBlockchain.createBlock({transactions: transactions});
            const status = await testBlockchain.pushBlock(block);
            expect(status).toBe(FullChain.ERR_INVALID);
        })().then(done, done.fail);
    ...
    it('prevents transaction replay across blocks', (done) => {
        (async function () {
            ...
            const transactions = [TestBlockchain.createTransaction(senderPubKey, receiverAddr, 1, 0, senderPrivKey)];
    ...
    it('can push 10 blocks with constant difficulty, then increase the difficulty over 10 more blocks', (done) => {
        (async function () {
            ...
            let timestamp;
            for (let i = 0; i < 10; ++i) {
                timestamp = testBlockchain.height * Policy.blockTime(2);
                const block = await testBlockchain.createBlock({timestamp: timestamp});
            ...
            for (let i = 0; i < 10; ++i) {
                const block = await testBlockchain.createBlock({timestamp: 10 * Policy.blockTime(2) + i});
                const hash = block.hash();
                const status = await testBlockchain.pushBlock(block);
                expect(status).toBe(FullChain.OK_EXTENDED);
    ...
    it('can push 20 blocks and keep difficulty increasing over each block', (done) => {
        (async function () {
            ...
            let difficulty = 0;
            for (let i = 0; i < 20; ++i) {
                let timestamp = testBlockchain.height * Math.floor(Policy.blockTime(2) / 2);
                const block = await testBlockchain.createBlock({timestamp: timestamp});
                const hash = block.hash();
                const status = await testBlockchain.pushBlock(block);
                expect(status).toBe(FullChain.OK_EXTENDED);
    ...
    it('changes balance after transaction', (done) => {
        (async () => {
            ...
            const tx1 = TestBlockchain.createTransaction(user0.publicKey, user1.address, 1, 9, user0.privateKey);
            const tx2 = TestBlockchain.createTransaction(user0.publicKey, user1.address, 1, 10, user0.privateKey);
    ...
    it('cannot push blocks with transactions to oneself', (done) => {
        (async function () {
            ...
            const transaction = TestBlockchain.createTransaction(user.publicKey, user.address, 1, 1, user.privateKey);
    ...
    it('updates transactions cache on rebranch', (done) => {
        (async function () {
            ...
            const tx1 = TestBlockchain.createTransaction(users[0].publicKey, users[1].address, 2000, 1, users[0].privateKey);
            const tx2 = TestBlockchain.createTransaction(users[0].publicKey, users[1].address, 1000, 1, users[0].privateKey);
            const tx3 = TestBlockchain.createTransaction(users[0].publicKey, users[1].address, 500, 2, users[0].privateKey);
            const tx4 = TestBlockchain.createTransaction(users[0].publicKey, users[1].address, 1, 1, users[0].privateKey);
            const tx5 = TestBlockchain.createTransaction(users[1].publicKey, users[0].address, 500, 2, users[1].privateKey);
            const tx6 = TestBlockchain.createTransaction(users[1].publicKey, users[0].address, 200, 2, users[1].privateKey);
    ...
    it('correctly creates TransactionsProofs by address', (done) => {
        (async function () {
            ...
            const tx1 = TestBlockchain.createTransaction(user0.publicKey, user1.address, 1, 1, user0.privateKey);
            const tx2 = TestBlockchain.createTransaction(user0.publicKey, user2.address, 1, 1, user0.privateKey);
            const tx3 = TestBlockchain.createTransaction(user0.publicKey, user3.address, 1, 1, user0.privateKey);
            const tx4 = TestBlockchain.createTransaction(user0.publicKey, user4.address, 1, 1, user0.privateKey);
