...
Client.Mempool = class Mempool {
    ...
    async getStatistics() {
        ...
        return new Client.MempoolStatistics(consensus.getMempoolContents(), height);
    }
...
Client.MempoolStatistics = class MempoolStatistics {
     ...
     */
    constructor(mempoolContents, height) {
        ...
        for (const tx of mempoolContents) {
            ...
            if (this._totalSize < size) {
                this._requiredFeePerByte = tx.feePerByte;
            }
        }
        if (this._totalSize < size) this._requiredFeePerByte = 0;
    }
