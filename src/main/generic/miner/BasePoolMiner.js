...
class BasePoolMiner extends Miner {
     ...
     * @param {BigNumber} confirmedBalance
     * @param {boolean} payoutRequestActive
     * @private
     */
    _onBalance(balance, confirmedBalance, payoutRequestActive) {
        const oldBalance = this.balance || 0, oldConfirmedBalance = this.confirmedBalance || 0;
        this.balance = new BigNumber(balance);
        this.confirmedBalance = new BigNumber(confirmedBalance);
        this.payoutRequestActive = payoutRequestActive;
        if (!this.balance.eq(oldBalance) || !this.confirmedBalance.eq(oldConfirmedBalance)) {
            Log.i(BasePoolMiner, `Pool balance: ${Policy.satoshisToCoins(this.balance).toString()} BTCK (confirmed ${Policy.satoshisToCoins(this.confirmedBalance).toString()} BTCK)`);
        }
        if (!this.balance.eq(oldBalance)) {
            this.fire('balance', balance);
        }
        if (!this.confirmedBalance.eq(oldConfirmedBalance)) {
