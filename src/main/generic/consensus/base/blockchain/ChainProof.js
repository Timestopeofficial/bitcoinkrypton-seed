class ChainProof {
    ...
    _verifyDifficulty() {
        ...
        if (headIndex >= 0 && denseChain[headIndex].height < Policy.difficultyBlockWindow(denseChain[headIndex].height)) return true;
        while (headIndex >= 0) {
            let tailIndex = headIndex - Policy.difficultyBlockWindow(denseChain[headIndex].height);
            if (tailIndex < 0) {
                if (denseChain[0].height === 1) tailIndex = 0;
                else break;
            }
