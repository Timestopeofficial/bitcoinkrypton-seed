...
class BaseChain extends IBlockchain {
    ...
    async getNextTarget(block, next) {
        ...
        // Retrieve the timestamp of the block that appears blockWindow blocks before the given block in the chain.
        // The block might not be on the main chain.
        const blockWindow = Policy.difficultyBlockWindow(block.height);
        const tailHeight = Math.max(block.height - blockWindow, 1);
        /** @type {ChainData} */
        let tailData;
        if (headData.onMainChain) {
            ...
            for (let i = 0; i < blockWindow && !prevData.onMainChain; i++) {
